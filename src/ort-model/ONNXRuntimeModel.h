...
public:
	// default constructor
	ONNXRuntimeModel() : input_tensor_type_(ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT) {}
	...
	const std::string& getProvider() const { return use_gpu; }

	virtual std::vector<Object> inference(const cv::Mat &frame) = 0;

	// Allow JSON config to override shape after construction
	virtual void setOutputShape(int num_detections, int features_per_det) { (void)num_detections; (void)features_per_det; }
...
protected:
	...
	int device_id_;
	std::string use_gpu;
	ONNXTensorElementDataType input_tensor_type_;
	std::vector<size_t> output_buffer_size_;
	const std::vector<float> mean_ = {0.485f, 0.456f, 0.406f};
	const std::vector<float> std_ = {0.229f, 0.224f, 0.225f};
	float input_normalize_scale_ = 1.0f;
